

export const ProjectList = [
  {
    title:"Versatime",
    github: "https://github.com/jcbtmy/Versatime",
    thumbnail: "images/serialNumbers.gif",
    description: `Serial number tracking system for VersaCall technologies`,
    libs : ["React", "NodeJS", "Docker", "Jenkins"],
  },
  {
    title: "Surf Waves Application",
    demo: "/demo/surfapp",
    github: "https://github.com/jcbtmy/SurfApp",
    thumbnail: "/images/surf.gif",
    description: `
      This application utilizes data from the National Data Buoy Centers API to figure out surf heights and temperatures
    `,
    libs :["React","NBDC API", "XML"], 
  },
      {
        title:"Cellular Automata Hash",
        demo: "/demo/cellular-hashing",
        thumbnail: "images/rule.png",
        description: `API to hash input with cellular automata. Returns output, image, and different rules for hashes`,
        libs : ["C#" , ".NET", "MongoDB","React"],
      },

      {
        title:"Cellular Automata Background",
        demo: "/demo/cellular-background",
        thumbnail: "images/ca.gif",
        description: `2D cellular automata simulation made with React states.`,
        libs : ["React", "Javascript"],
      },
      {
        title : "Cloud Animations",
        demo: "/demo/clouds",
        thumbnail: "/images/clouds.gif",
        github: "",
        libs : ["Javascript"],
        description: `
          Randomized cloud animations with javascript and css
        `,
      },

];